...
import {
  LineChart, Line, BarChart, Bar, CartesianGrid, XAxis, YAxis, Tooltip,
...
/** ---------- domain data ---------- */
const CONDITIONS = [
  "Alzheimer's","Dementia","Parkinson's","MCI","Depression","Anxiety","PTSD","Bipolar",
];
...
function genTrend(condition, weeks=12){
  const def=METRIC_DEFS[condition]; const arr=[];
  for(let i=0;i<weeks;i++){
    ...
    arr.push({week:`W${i+1}`,[def.primary.key]:Math.max(0,v1),[def.secondary.key]:Math.max(0,v2)});
  }
  return arr;
}
function genRadar(condition){
  const def=METRIC_DEFS[condition];
  return def.radar.map((axis,i)=>({axis,score:Math.round(40+seededRand(i+condition.length)*60)}));
}

const NOTES=[
  {date:"2025-09-25",title:"Follow-up visit",tags:["Memory","Medication"],
   body:"Patient reports mild forgetfulness in daily tasks. Started donepezil 5mg nightly last week; tolerating well. Sleep improved. Caregiver notes fewer repeated questions."},
  {date:"2025-09-10",title:"Caregiver phone check-in",tags:["Caregiver","Behavior"],
   body:"Caregiver reports increased agitation in evening hours. Introduced calming routine at 7pm and limited caffeine. Plan to reassess; consider melatonin if persists."},
  {date:"2025-08-28",title:"Initial evaluation",tags:["Assessment","Plan"],
   body:"Baseline cognitive screen. MMSE 24/30, MoCA 22/30. Labs ordered (TSH, B12). Safety and medication education provided."},
];
  ...
  return (
    <div className="rounded-xl border border-[#6b4b3e] bg-white p-4 text-[#6b4b3e]">
      ...
      <div className="mt-1 text-2xl font-bold">
        {value}
        <span className="ml-1 text-base font-medium opacity-60 text-[#6b4b3e]">{unit}</span>
      </div>
    </div>
  );
}
          ...
          <div className="h-56">
            <ResponsiveContainer>
              <LineChart data={trend}>
                ...
                <Line type="monotone" dataKey={def.primary.key} strokeWidth={2} dot={false} stroke={PALETTE.darkBrown}/>
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
...
function PatientNotes(){
  const [active,setActive]=useState(0); const note=NOTES[active];
  return (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3">
              ...
              <li key={n.date}>
                <button onClick={()=>setActive(i)}
                  className={`w-full rounded-lg border px-3 py-2 text-left transition-colors ${
                    active===i?"border-[#6b4b3e] bg-[#6b4b3e] text-white"
                    :"border-[#6b4b3e] bg-white text-[#6b4b3e] hover:bg-[#c49e85] hover:text-white"}`}>
                  <div className="text-sm font-semibold">{new Date(n.date).toLocaleDateString()}</div>
        ...
        <div className="p-4 md:col-span-2">
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            {note.tags.map(t=>(
              <span key={t} className="inline-block rounded-full border border-[#6b4b3e] bg-white px-2 py-1 text-xs text-[#6b4b3e]">{t}</span>
            ))}
          </div>
      ...
      {tabs.map(t=>(
        <button key={t} onClick={()=>onChange(t)}
          className={`h-10 rounded-full border px-4 text-sm font-semibold transition-colors ${
            t===value?"border-[#6b4b3e] bg-[#6b4b3e] text-white"
            :"border-[#6b4b3e] bg-white text-[#6b4b3e] hover:bg-[#c49e85] hover:text-white"}`}>
          {t}
        </button>
      ))}
...
function buildCSV({ patient, condition, def, trend, ai }){
  ...
  const trendHeader = `\nTrend,\n${["Week",def.primary.key,def.secondary.key].map(csvEscape).join(",")}\n`;
  const trendRows = trend.map(r => [r.week, r[def.primary.key], r[def.secondary.key]].map(csvEscape).join(",")).join("\n");

  return `${head}${summary}${notes}${trendHeader}${trendRows}\n`;
}

function RiskBadge({ score, level }){
  const color = level==="High"?"bg-red-600":level==="Medium"?"bg-yellow-500":"bg-emerald-600";
  return (
    <span className={`inline-flex items-center gap-2 rounded-full px-3 py-1 text-white ${color}`}>
      Priority: {level} <span className="rounded bg-white/20 px-2 py-[2px] text-white">{score}</span>
    </span>
  );
}

function AIAssistantPanel({ exportRef, patient, condition, def, trend, radar, notes }){
  ...
  async function generate(){
    ...
    try{
      const res=await fetch("/api/ai/cog-summary",{method:"POST",headers:{"Content-Type":"application/json"},
        body:JSON.stringify({ patient, condition, primaryKey:def.primary.key, secondaryKey:def.secondary.key, trend, radar, notes })});
  ...
  function exportCSV(){
    ...
    a.href=url; a.download=`${patient?.name ?? "patient"}_${condition}_summary.csv`;
    document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url);
  }

  async function exportPDF(){
    // dynamic import so it never breaks SSR
    ...
    // Add snapshot of the dashboard area
    if (exportRef?.current){
      const canvas=await html2canvas(exportRef.current, { scale: 2, useCORS:true });
      const imgData=canvas.toDataURL("image/png");
      const pageWidth=doc.internal.pageSize.getWidth()-80; // 40 margins
      ...
      if(imgH <= pageHeight){
        ...
        while (sY < canvas.height){
          ...
          ctx.drawImage(canvas, 0, sY, canvas.width, part.height, 0, 0, part.width, part.height);
          const partData = part.toDataURL("image/png");
          doc.addImage(partData,"PNG",40,y,imgW, part.height * (imgW / part.width));
          sY += sliceH;
          if (sY < canvas.height) { doc.addPage(); y = 40; }
        }
      }
    }
    ...
    if (result){
      ...
      doc.setFontSize(11);
      const wrap = (txt)=>doc.splitTextToSize(txt, doc.internal.pageSize.getWidth()-80);
      doc.text(wrap(result.summary||""), 40, 60);

      if(result.drivers?.length){
        doc.setFontSize(12); doc.text("Key drivers",40, 140);
        doc.setFontSize(11);
        result.drivers.forEach((d,i)=>doc.text(`• ${d}`, 40, 160 + i*16));
      }
      if(result.recommendations?.length){
        const base = 160 + (result.drivers?.length||0)*16 + 20;
        doc.setFontSize(12); doc.text("Recommended next steps",40, base);
        doc.setFontSize(11);
        result.recommendations.forEach((d,i)=>doc.text(`• ${d}`, 40, base + 20 + i*16));
      }
      doc.setFontSize(10);
      doc.text(`Priority: ${result.risk_level} (${result.risk_score}) · Confidence ${(result.confidence*100).toFixed(0)}% · Source: ${result.source}`, 40, doc.internal.pageSize.getHeight()-40);
    }
        ...
        <div className="mt-3 space-y-3">
          ...
          {result.recommendations?.length>0 && (
            <div>
              ...
              <ul className="list-disc space-y-1 pl-5 text-sm">
                {result.recommendations.map((d,i)=><li key={i}>{d}</li>)}
              </ul>
            </div>
          )}
  ...
  return (
    <main className="min-h-screen bg-[#f8f4f9] p-6 text-[#6b4b3e] md:p-10">
      <header className="mb-6">
        ...
        <div className="mt-1 text-sm opacity-80">MRN ••••1234 · DOB 1959-02-17 · Last visit 2025-09-25</div>
      </header>

      {/* Everything below is wrapped so html2canvas can snapshot it */}
      <div ref={exportRef}>
