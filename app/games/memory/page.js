...
const TILES = [
  { id: 0, label: "C", symbol: "♪", freq: 261.63, baseClass: "bg-[#F7A6A1]/70 text-[#4b2f23]", activeClass: "bg-[#F7A6A1] text-[#4b2f23]", legend: "#F7A6A1" },
  { id: 4, label: "D", symbol: "♪", freq: 293.66, baseClass: "bg-[#FBC4A0]/70 text-[#4b2f23]", activeClass: "bg-[#FBC4A0] text-[#4b2f23]", legend: "#FBC4A0" },
  { id: 1, label: "E", symbol: "♪", freq: 329.63, baseClass: "bg-[#FDF1A9]/70 text-[#4b2f23]", activeClass: "bg-[#FDF1A9] text-[#4b2f23]", legend: "#FDF1A9" },
  { id: 2, label: "G", symbol: "♪", freq: 392.0, baseClass: "bg-[#B6E2B6]/70 text-[#4b2f23]", activeClass: "bg-[#B6E2B6] text-[#4b2f23]", legend: "#B6E2B6" },
  { id: 5, label: "A", symbol: "♪", freq: 440.0, baseClass: "bg-[#A7D8F7]/70 text-[#4b2f23]", activeClass: "bg-[#A7D8F7] text-[#4b2f23]", legend: "#A7D8F7" },
  { id: 3, label: "B", symbol: "♪", freq: 493.88, baseClass: "bg-[#C7B5E9]/70 text-[#4b2f23]", activeClass: "bg-[#C7B5E9] text-[#4b2f23]", legend: "#C7B5E9" },
];
...
function useAudio() {
  ...
  function play(freq = 440, duration = 350, volume = 0.2) {
    ...
    return { stop: () => { try { osc.stop(); gain.disconnect(); osc.disconnect(); } catch {} } };
  }
  return { play, resume };
}
...
export default function MemorySequenceGame() {
  ...
  useEffect(() => { localStorage.setItem("memory.best", String(best)); }, [best]);
  useEffect(() => { sequenceRef.current = sequence; }, [sequence]);

  function randomTile() { return TILES[Math.floor(Math.random() * TILES.length)].id; }
  ...
  async function playBack() {
    ...
    for (let i = 0; i < seq.length; i++) {
      ...
      await flashTile(seq[i], Math.floor(base * 0.75));
  ...
  function stopPlayback() { playingRef.current = false; }
  ...
  return (
    // Wrapper keeps the cozy background behind the sticky MainNav as well
      <div className="relative">
          {/* 👇 Keeps the background across the whole page, even while scrolling */}
          <div className="fixed inset-0 -z-10 bg-[#f8f4f9]" />

          {/* Optional decorative blobs pinned behind content */}
      ...
      <main
        className="relative py-10 px-4 md:px-10 text-[#4b2f23]"
      ...
      >
        ...
        <div className="relative z-10 mx-auto flex max-w-5xl flex-col space-y-8">
          ...
          <section className="rounded-2xl border border-[rgba(107,75,62,0.15)] bg-white/85 p-6 shadow-sm space-y-4">
            ...
            <div className="flex flex-wrap gap-4 text-base text-[#4b2f23]">
              ...
              <span className="inline-flex items-center gap-2 rounded-full bg-white/80 px-4 py-1 font-semibold" aria-live="polite">
                Status <span className="font-medium text-[#4b2f23] opacity-80">{message}</span>
              </span>
            </div>
                ...
                return (
                  <button
                    ...
                    className={[
                      ...
                      status === "computer" ? "opacity-70 cursor-not-allowed" : "hover:brightness-105 active:scale-[0.98]",
                    ].join(" ")}
                  >
          ...
          <footer className="max-w-3xl text-sm leading-relaxed text-[#4b2f23] opacity-80">
            ...
            otherwise the round is replayed so you can learn the pattern. Your best score is saved locally.
          </footer>
        </div>
      </main>
    </div>
