...
const STEPS = [
  ...
  { id: "smile", text: "Simon says open your mouth" },
...
function Page() {
  ...
  return (
    <div className="relative min-h-screen">
      {/* Full-page background (stays during scroll) */}
      ...
      {/* Content */}
      <main className="mx-auto max-w-5xl px-4 md:px-10 py-10 text-[#4b2f23]">
        <div className="relative z-10 mx-auto flex flex-col space-y-8">
...
function LV({ label, value }) {
  return (
    <div className="rounded-xl border border-[#c49e85]/50 bg-[rgba(255,214,175,0.4)] px-3 py-2 text-[#4b2f23]">
      <div className="text-[11px] uppercase tracking-wide text-[#4b2f23] opacity-60">
        {label}
      </div>
...
function drawGuides(ctx, k) {
  ...
  ctx.lineWidth = 3;
  ctx.strokeStyle = "rgba(220,0,70,0.9)";
  ctx.strokeRect(minX - 8, minY - 8, maxX - minX + 16, maxY - minY + 16);

  const L = { top: 159, bot: 145, left: 33, right: 133 },
    R = { top: 386, bot: 374, left: 362, right: 263 },
    M = { top: 13, bot: 14, left: 61, right: 291 };
...
function ratio(top, bot, left, right) {
  ...
  const v = dist(top, bot),
    h = dist(left, right);
  return h ? v / h : 0;
}
function mid(a, b) {
  return !a || !b ? { x: 0, y: 0 } : { x: (a.x + b.x) / 2, y: (a.y + b.y) / 2 };
}
function dist(a, b) {
  if (!a || !b) return 0;
  const dx = a.x - b.x,
...
function drawHUD(ctx, { m, step, consec, needed }) {
  const text = `Step: ${step} | EAR ${m.EAR.toFixed(2)}  MAR ${m.MAR.toFixed(2)}  EB ${m.EB.toFixed(
    0
  )} | ${consec}/${needed}`;
